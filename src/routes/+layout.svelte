...
<header
	class="sticky top-0 z-10 grid w-full px-5 py-4 bg-white md:mx-auto max-2xl:border-b max-w-screen-2xl md:flex md:items-center md:justify-between md:px-20"
>
	<a href="/">
		<img
			src="/svg/logo-{data.page.data.color === true ? 'v2' : 'v1'}.svg"
			class="w-48 md:w-60 lg:w-72"
			alt="logo-header"
		/>
	</a>
	...
	<footer class="flex flex-col items-center px-5 py-5 mt-20 shadow-inner md:mx-auto md:px-24">
		...
		<div class="flex justify-between w-full md:gap-6 md:justify-center">
			...
			<div class="mb-8 text-right infos-contact">
				<a
					href="https://www.google.com/maps/search/?api=1&query={data.settings.data.googlemap}"
					target="_blank"
					class="mb-1"><PrismicRichText field={data.settings.data.adress} /></a
				>
				<a href="tel:{data.settings.data.telephone}" class="block telephone hover:underline"
					>{data.settings.data.telephone}</a
				>
			</div>
		</div>
		<div class="text-[.55rem] mt-8 md:text-xs footer__copyright">
			©{currentYear}
			{siteName} | tous droits réservés |
			<a href="/mentions-legales" class="text-[.55rem] md:text-xs hover:underline"
				>mentions légales</a
			>
		</div>
	</footer>
